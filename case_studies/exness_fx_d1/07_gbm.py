...
# %% [markdown]
# # Exness FX D1: what a tree can express that a weighted sum cannot
#
# [`06_linear`](06_linear.ipynb) established two things about this universe. The five pairs all
# have the dollar on one side, so the cross-section is dependent - a dollar move appears in every
# pair at the same moment, with the sign set by which side the dollar is quoted on, and `01`
# counts 2.4 independent bets among the five - and a penalty addresses the collinear feature set
# while doing nothing about that. What the penalty sweep did to the ranking is in that notebook's
# horizons frame, and it is the baseline this one is read against.
#
...
# otherwise rank on the mean-reversion z-score" without anyone constructing that column. This bot
# has no carry column (`bots/exness_fx_d1/BOT.md`, Decisions log), so the interaction the template
# names - carry against momentum - is not on the table; the dollar-regime HMM state and the
# `usd_corr_*` and `gold_corr_*` columns from `03` and `04` are the conditions a tree can split
# on instead. Whether an exchange-rate cross section rewards that is the question here, and it is
# a fair one: currency returns are widely modelled as regime-dependent, and a regime is exactly a
# condition on one variable that changes how another behaves.
#
...
# (`ML4T_OUTPUT_DIR=experiments/exness_fx_d1` and `WORKSPACE` naming the same directory) on the
...
# %%
"""Fit the declared exness_fx_d1 gradient boosting population on the walk-forward folds."""
...
# contention was measured (bots/exness_fx_d1/BOT.md, Decisions log); `modeling.gbm.num_threads: 2`
...
# %%
study = open_study("exness_fx_d1", execution_tier=EXECUTION_TIER, workspace=WORKSPACE or None)
...
# %%
population_name = POPULATION_NAME or "exness_fx_d1-gbm-validation-v1"
...
#
# ```python
# study = open_study("exness_fx_d1", workspace="~/ml4t-experiments")
# configs = load_model_configs(
...
last_points = curves.filter(
    pl.col("checkpoint_value") == pl.col("checkpoint_value").max().over("label")
)
ends_above = "; ".join(
    f"{label}: "
    f"{int((last_points.filter(pl.col('label') == label).get_column('ic_mean') > 0).sum())} of "
    f"{curves.filter(pl.col('label') == label).n_unique('config_name')} lines end above zero"
...
show_plotly_with_alt(
    ...
    f"dashed zero line. At the last checkpoint, {ends_above}.",
)
...
final_above = "; ".join(
    f"{label}: "
    f"{int((final.filter(pl.col('label') == label).get_column('ic_mean') > 0).sum())} of "
    f"{final.filter(pl.col('label') == label).height} bars above zero"
    for label in panel_labels
)
show_plotly_with_alt(
    fig_obj,
...
# ## 5. What to notice
#
# Every number below is read from the frames above as they came out of the first published run
# (WSL2, 2026-09-05, experiment `experiments/exness_fx_d1`, population
# `exness_fx_d1-gbm-validation-v1`, 45 configurations, 450 prediction sets, CPU, two threads).
# The template's reading of twenty G10 pairs is in `case_studies/fx_pairs/07_gbm.ipynb`; this
# bot's data decides where the two disagree.
#
# **Four Huber configurations produce no ranking at all at the traded horizon.** `leaves_7_huber`,
# `leaves_15_huber`, `leaves_31_huber` and `leaves_63_huber` on `fwd_ret_1d` have `ic_mean` null
# at every one of their ten checkpoints: the regularized presets (`min_child_samples 50`,
# `lambda_l1 0.5`, `lambda_l2 5`) with a Huber threshold at half the one-day label's standard
# deviation - a few basis points - find no split worth its penalty, and a booster that never
# splits predicts one number for every pair on every date. A constant has no rank correlation
# with anything, so those 40 prediction sets are complete, registered, in the population, and
# undiagnosable. `default_huber`, which carries no regularization, is not affected, and neither
# is any Huber fit at five or 21 days, where the label's scale is larger. The `horizons` and
# `trees_effect` frames therefore count 11 configurations at one day and 15 at the other two.
#
# **At one day the tree ensemble sits where the linear grid sits: above zero by a thickness no
# test can see.** At the final iteration the best of the eleven diagnosable configurations is
# `leaves_31_mae` at +0.013 (HAC t 0.79, p 0.43; fold ICs +0.000, +0.019, +0.054, -0.021 from the
# most recent validation year backwards, three of four positive), six of eleven are above zero,
# and the five squared-error fits are the five lowest, at -0.014 for four of them. The
# `objective_summary` ordering is absolute error (+0.006 mean, four of five above zero) ahead of
# Huber (one diagnosable fit, +0.005) ahead of squared error (-0.011, one of five above zero).
# The template had two of fifteen above zero here; this bot has six of eleven, and none of the
# 150 one-day checkpoint rows reaches |HAC t| >= 1.96. The comparison against `06_linear` is
# +0.013 against +0.006, both with a HAC t below one: the tree ensemble is not worse than the
# linear baseline at this horizon, and neither is distinguishable from zero.
#
# **Three squared-error presets are the same model.** `leaves_15_mse`, `leaves_31_mse` and
# `leaves_63_mse` post identical ICs at every checkpoint on every label (-0.01361 at one day),
# and `leaves_31_huber` equals `leaves_63_huber` at five and 21 days. With 3,870 training rows
# and `min_child_samples 50` a tree cannot use more than a few dozen leaves, so raising the cap
# from 15 to 63 changes nothing the fit can reach. Capacity above 15 leaves is not a free
# parameter on a panel this size; those presets count as trials because they were declared and
# fitted, but they add no distinct candidate.
#
# **The longer horizons rank the cross-section backwards, and more trees make it worse.** Every
# configuration is negative at five and 21 days at every checkpoint. At 21 days the median
# configuration loses 0.034 of IC between its first and last checkpoint, all fifteen end lower
# than they started and none peaks in the interior: the curve of a model fitting the training
# window's dollar path ever more closely and being wrong about the next one. The worst row in
# the whole grid is `default_mse` at 21 days, -0.127 with HAC t -2.41 (p 0.016), and nine of the
# 150 21-day checkpoint rows and five of the five-day ones clear |HAC t| >= 1.96, all on the
# negative side. `06_linear` found the same sign at both horizons with smaller magnitudes; a
# reversed relationship that a higher-capacity model fits more confidently is the reading that
# fits both.
#
# **The horizon effect of the template does not reproduce, in the same direction as `06`.** The
# template's best IC and count above zero rose with the horizon; here `best_ic` is +0.013 at one
# day, -0.028 at five and -0.065 at 21, and `above_zero` is 6, 0 and 0.
#
# **The tails do not explain the objective ordering here either.** Excess kurtosis on the
# development window is 3.8 at one day, 4.5 at five and 1.5 at 21. The ordering absolute error
# > Huber > squared error holds at one day; at five days Huber leads (-0.032), then squared
# error (-0.044), then absolute error (-0.049); at 21 days Huber, absolute, squared. The
# heaviest-tailed label is the five-day one and it is where absolute error does worst, so the
# tail mechanism the template discusses is not what separates the losses on this panel.
#
# **The checkpoint is a decision of the same order as the model, as in the template.** The
# `comparison` frame puts the spread across configurations at fixed length against the median
# configuration's own range across ten checkpoints at 2.0 (one day), 1.8 (five) and 1.7 (21).
# The best one-day number anywhere in the grid is `default_huber` at 100 trees, +0.018 (HAC t
# 1.13, p 0.26), and it is +0.005 at 500: choosing that checkpoint after seeing the folds would
# be worth more than choosing the model, which is why the checkpoint travels into `13_backtest`
# as part of what is selected rather than being resolved here.
#
# **What this says for the bot.** No linear or boosted configuration shows a validation IC
# distinguishable from zero at `fwd_ret_1d`, and both families rank the five- and 21-day
# cross-sections backwards. That is the same reading `05_evaluation` gave the single features and
# the `02_labels` baseline. It is a result: the draft hypothesis in `bots/exness_fx_d1/BOT.md`
# is not supported by the ranking evidence on the four validation years. The backtest still
# runs, on the validation split, over all 534 members of the two populations, because a top-1 /
# top-2 long-short book over five names can earn what an IC over five points cannot show - and
# because the cost stage, not this notebook, is where "small" becomes "too small".
#
# **None of this selects anything.** Selection is on validation backtest Sharpe over the
# population just published, in `13_backtest`, where the checkpoint is part of what is selected.
#
# **Known limitations.** The IC is an average of daily rank correlations over five names with
# 2.4 independent bets among them, so each daily value is coarse and the effective sample behind
# a HAC t is smaller than the date count suggests; the five- and 21-day labels overlap across
# consecutive dates; the grid varies capacity and loss at one learning rate and one training
# length, and on this panel three of its five capacity settings coincide; four one-day Huber
# fits are constant and cannot be compared at all; `num_threads: 2` is part of every training
# identity here (Decisions log), so re-fitting on another host under a different thread count
# registers new hashes beside these. And every number is measured on validation folds already
# read by `05` and `06`.
#
# **Next**: the tabular and sequence DL stages of the template (`08`-`10a`) are not copied yet;
# `12_model_analysis` reads the two populations published so far, and `13_backtest` selects.
