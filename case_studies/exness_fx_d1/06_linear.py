...
# %% [markdown]
# # Exness FX D1: regularizing a five-pair dollar cross-section
#
# The five pairs in this universe are not five independent assets. Every one of them has the
# dollar on one side - `EURUSD`, `GBPUSD` and `AUDUSD` quote a currency against it, `USDJPY` and
# `USDCAD` quote it against a currency - so a dollar move shows up in all five at the same moment,
# with the sign set by which side the dollar is quoted on. `01_feasibility_analysis` counts 2.4
# independent bets among the five (the template's twenty G10 pairs count 5.3), and the
# `usd_corr_*` state columns that `05_evaluation` put at the top of its screen say the same thing
...
# and this universe has those too - seven return windows, six volatility windows, seven rolling
# Sharpe ratios, eight cross-sectional ranks computed from them, and `05_evaluation` collapsed
# its 52 scored columns into eleven redundancy groups. **A dependent cross-section** is something
# else: the rows are linked, so ranking the pairs against each other is ranking overlapping bets
# rather than distinct ones. A model that discovers "the dollar is strong" scores well on the
# ranking and has found one fact, not five.
#
# **Regularization** - adding a penalty on coefficient size to the fitting objective - addresses
# the first problem directly and the second not at all. This notebook runs the penalty sweep the
# template [`fx_pairs/06_linear`](../fx_pairs/06_linear.ipynb) runs, on this bot's own labels
# and features, and the results are read with that distinction in mind. The design matrix is the
...
# (`ML4T_OUTPUT_DIR=experiments/exness_fx_d1` and `WORKSPACE` naming the same directory, where
...
# %%
"""Fit the declared exness_fx_d1 linear-model population on the walk-forward validation folds."""
...
# %%
population_name = POPULATION_NAME or "exness_fx_d1-linear-validation-v1"
...
if ridge.height:
    ...
    show_plotly_with_alt(
        ...
        "the Ridge penalty, one line per label, against a dashed zero line. The peak on each line "
        f"is ringed: {peak_text}.",
    )
else:
    print(
...
# ## 5. What to notice
#
# Every number below is read from the frames above as they came out of the first run of this
# notebook (WSL2, 2026-09-05, experiment `experiments/exness_fx_d1`, population
# `exness_fx_d1-linear-validation-v1`, 84 configurations, 336 folds fitted). The template's
# reading of twenty G10 pairs on OANDA bars is in `case_studies/fx_pairs/06_linear.ipynb` and is
# the prior this run is compared against; where the two disagree, this bot's data wins.
#
# **Nothing in the grid is distinguishable from zero at the traded horizon.** On `fwd_ret_1d`
# the best full-coverage configuration is `ridge_a1.0` at IC +0.006 (HAC t 0.37, p 0.71), eight
# of 26 charted configurations are above zero, and the unpenalized fit is +0.003. No
# configuration on any label reaches |HAC t| >= 1.96 in the positive direction. The fold ICs of
# the leader, most recent validation year first (2024-25 back to 2021-22), are -0.018, +0.067,
# +0.006, -0.031: positive on two of four validation years, which
# is short of the "positive on at least 3 of 4 folds" line the draft hypothesis in
# `bots/exness_fx_d1/BOT.md` sets for itself. That is a result, not a defect: it is the same
# reading `05_evaluation` gave the single features (no column clears the BH-FDR screen) and the
# `02_labels` baseline (trailing return, IC -0.002).
#
# **Shrinkage does not do here what it did in the template.** In `fx_pairs` the Ridge curve rose
# from below zero at weak penalties to a peak at the strong end. Here the one-day curve peaks at
# alpha = 1 (+0.006), sits at +0.003 to +0.005 from OLS up to alpha = 100, and goes *negative*
# from alpha = 10,000 on (-0.019 at 1e4). With five names and 67 columns the unpenalized fit is
# not chasing a reversed relationship the way it did with twenty; the heavy penalties instead
# flatten the coefficients towards the columns that carry the dollar factor, and that ranks the
# cross-section backwards. The `horizons` frame says the same at the other two labels: the least
# negative configuration is a sparse one (`enet_f0.7` at 21 days, `enet_f0.015` at five) and the
# heavily penalized Ridge fits are the worst (-0.080 and -0.034).
#
# **The longer horizons are worse, not better.** The template found the best IC and the count
# above zero rising with the horizon. Here `above_zero` is 8 at one day and 0 at five and 21
# days: every one of the 28 configurations ranks the five- and 21-day cross-sections backwards.
# Four five-day configurations, the most aggressive L1 settings (`lasso_f0.5`, `enet_f0.5`,
# `lasso_f0.85`, `enet_f0.85`), post HAC t between -1.96 and -2.20, which is about what 28 tests
# at the 5 % level would produce by chance if the direction were random, and the overlap of the
# five-day windows makes the HAC t at that horizon a less precise number than at one day. Read
# it as "the sparse fits pick a few columns whose sign reverses out of sample", not as a
# tradable reversed signal.
#
# **The horizons agree on the one-to-five-day ordering and on nothing else.** The `transfer`
# frame puts the rank correlation of the configuration ordering at 0.81 between one and five
# days, 0.08 between one and 21 days and -0.14 between five and 21. A penalty that is least bad
# at one day is least bad at five; the 21-day ordering is unrelated to both.
#
# **Coverage.** 26 of 28 configurations at one and five days scored every date their label
# offers (1,031 and 1,027); `lasso_f0.85` and `enet_f0.85` zero every coefficient on the third
# fold and are marked `full_coverage` false (773 and 769 dates), as the template's most
# aggressive L1 settings were. At 21 days all 28 score all 1,011 dates.
#
# **A penalty cannot fix a dependent cross-section.** With five names every rank correlation is
# computed over five points, and `01` counts 2.4 independent bets among them. A prediction that
# ranks the three "currency-against-dollar" pairs one way and the two "dollar-against-currency"
# pairs the other is one bet expressed five times. That is a property of the universe rather
# than of the model, it is not visible in an IC, and it is why `13_backtest` puts these
# predictions through a top-1 / top-2 long-short book rather than reading the ranking as a
# result.
#
# **None of this selects anything.** IC measures whether predictions rank pairs correctly, not
# whether a strategy trading them makes money after the measured 2.6 bp round trip. Selection is
# on validation backtest Sharpe over the population this notebook just published, in
# `13_backtest`, and every one of the 84 members goes there whatever its IC. The linear family
# is the baseline `07_gbm` is read against.
#
# **Known limitations.** The IC is an average of daily rank correlations over five names, so a
# single date's correlation takes one of a handful of values and the daily series is coarse;
# the five- and 21-day labels overlap across consecutive dates, so their HAC t statistics are
# less precise than the one-day ones at the same date count; the grid is a one-dimensional
# sweep of penalty strength at fixed features and folds; and every number is measured on the
# four validation years, which `05_evaluation` has already read once.
#
# **Next**: [`07_gbm`](07_gbm.ipynb) asks whether gradient boosting finds structure a linear
# model cannot represent - a dollar-regime condition on the momentum and mean-reversion columns
# - and is read against the numbers above.
