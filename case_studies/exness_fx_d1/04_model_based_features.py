...
# %% [markdown] tags=[]
# # Exness FX D1 (exness_fx_d1): Features Built From Fitted Models
#
...
# for the bot `exness_fx_d1` (`bots/exness_fx_d1/BOT.md`): the same three models on MT5
# history instead of OANDA, the five dollar pairs instead of twenty, and the session panel
...
# %% tags=[]
"""exness_fx_d1: Features Built From Fitted Models (Route B fork of fx_pairs)."""
...
# pairs: the floor is the one `01_feasibility_analysis` checks, twice the largest top-k the
# backtest sweep declares, so that a long leg and a short leg both exist to rank.
MIN_PAIRS_PER_DATE = 2 * max(max(ks) for ks in SETUP["backtest"]["sweep"]["top_k_grid"].values())
...
#
# The market divides quotes into two kinds. A **dollar pair** has the US dollar on one
# side of the quote, so its move is largely a move in the dollar itself; the dollar-regime
# model in section 5 is built from exactly these and no others, and in this universe that
# is every pair. A **cross** is quoted between two other currencies; there is none here,
# and the table still carries the group so a universe change shows up in it. `USDJPY` is
# quoted in hundredths rather than ten-thousandths, which puts its price on a different
# numeric scale from the other four in the file.
#
...
# %% [markdown] tags=[]
# The model reads one series: an average dollar return across the pairs that have the
# dollar on one side of the quote, which here is all five. The sign has to be fixed before
# averaging, because `USDJPY` rising and `EURUSD` rising are opposite moves in the dollar.
# Both sides are derived from the declared universe rather than listed here, so a universe
# change cannot silently drop a leg of the average; the names are the bare MT5 names, with
# the dollar as the first or the last three letters.

# %% tags=[]
USD_LONG = [s for s in SYMBOLS if s.startswith("USD")]
USD_SHORT = [s for s in SYMBOLS if s.endswith("USD")]
assert set(USD_LONG + USD_SHORT) == set(SYMBOLS), "a pair without the dollar on one leg"
...
record = write_artifact(
    ...
    written_by="case_studies/exness_fx_d1/04_model_based_features.py",
    inputs={
