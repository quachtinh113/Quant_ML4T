...
# %% [markdown]
# # Exness FX D1 (exness_fx_d1): Feature Evaluation
#
# The two feature stages built a matrix of candidate columns for the five dollar pairs of
# the bot `exness_fx_d1` (`bots/exness_fx_d1/BOT.md`; this is a Route B fork of
# [`fx_pairs/05_evaluation`](../fx_pairs/05_evaluation.ipynb), unchanged apart from the study
# id, the gold family and the size of the universe). This notebook asks the one question
# that has to be answered before any of them is fed to a model: taken on its own, does a
# column say anything about where the pair's price goes next?
#
...
# - Measure how well a column's ranking of the five pairs on one day agrees with their
...
# %%
"""exness_fx_d1: test each feature column on its own against the forward return it is built for."""
...
# pairs is mostly noise, so the floor is a quarter of the universe on the panel, with a
# minimum of three - which on five pairs is the floor itself, the smallest cross-section a
# rank correlation can be read on. Deriving it from what is loaded rather than typing it
# means a run reduced to fewer pairs lowers the floor with it, instead of discarding every
# session.
...
def assign_feature_family(feature_name: str) -> str:
    ...
    family_map = [
        ...
        (["gold_ret", "gold_corr"], "gold factor"),
...
# over five pairs takes five values, so a pair that stays near the top of a
...
# Of the 67 columns the feature stages produced, 59 are filled in and moving enough to
# test - the eight cross-sectional `rank_*` columns fail the staleness ceiling, because a
# percentile over five pairs takes five values and repeats itself on 68-98% of sessions -
# and 52 can rank the pairs, so 52 were scored over 4 folds. Three clear the five-percent
# level once Newey-West rescales their intervals (`usd_corr_21d`, `gold_corr_21d`,
# `usd_corr_63d`), and none clears the Benjamini-Hochberg adjustment over the set. The
# largest agreement in absolute terms is `usd_corr_21d`, at a mean of +0.0480 per session
# and a Newey-West t of +2.63, with the same sign in all four folds (fold means +0.022,
# +0.016, +0.066, +0.089): the strongest column on this panel, and still a candidate
...
#
# The bucket edges are cut **within** each session, from that session's five values
...
# effect-size floor, and it exists so that a five-pair cross-section, where almost
...
# %% [markdown] tags=["results"]
# The ledger records 34 PROCEED, 25 REVISE and 8 STOP decisions over the 67 columns.
# Every one of the 34 came through the exploration route (the same direction in at least
# 60% of four folds and an average agreement above 0.005), so nothing in this case study
# advances on a confirmed association, and the next stage starts from a set of candidates
# rather than from findings. Eight of the 34 keep one sign in all four folds and eight
# have a fold-level information ratio above one; the columns that do both are
# `usd_corr_21d`, `gold_corr_21d`, `usd_corr_63d`, `kalman_innovation`, `arima_residual`
# and `arima_residual_zscore` - the two cross-instrument exposures and the two surprise
# measures. With five pairs, one session's agreement is five observations, so the fold
# view is the evidence and the daily average is not.
...
# pairs, one session's agreement is estimated from five observations, so the daily
