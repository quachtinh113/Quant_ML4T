...
# > began as `case_studies/exness_fx_d1/04_model_based_features.py` with the study id, the loader and the population names
...
# %% [markdown] tags=[]
# # Exness US indices, session bot (exness_usidx_sess): Features Built From Fitted Models
#
...
#
# This is a Route B fork of [`fx_pairs/04_model_based_features`](../fx_pairs/04_model_based_features.ipynb)
# for the bot `exness_usidx_sess` (`bots/exness_usidx_sess/BOT.md`): the same three models on MT5
# history instead of OANDA, the five dollar indices instead of twenty, and the session panel
# built by the one shared function every stage of this case study uses.
#
# **Prerequisites**: MT5 four-hour bars, which section 1 aggregates to sessions through
# `_features.load_session_panel` (the same decision-bar rule `02_labels` sealed the labels on
# and `03_financial_features` builds its matrix on), and [`02_labels`](02_labels.ipynb), which
# writes the label parquet read in section 3 and whose date index the folds are derived from.
#
# **Output Contract**:
# - `features/model_based.parquet` -- ten columns, five from the state-space fit, two
#   from the market-regime fit and three from the return model
...
# %% tags=[]
"""exness_usidx_sess: Features Built From Fitted Models (Route B fork of exness_usidx_sess)."""
...
# how many indices are fitted, how many walk-forward windows are covered, how hard the
# search for the state-space noise parameters tries, and how many times the market-regime
# model is refitted from a different starting point before one of them is kept.
#
# `START_DATE` of `None` starts at `setup.yaml::universe.history_start` (2017-03-01), the
# first server day on which every pair carries the full six-bar four-hour grid, so it is the
# whole usable file rather than a choice about how much of it to use.
...
# A Spearman IC over fewer indices than this is a rank correlation over a handful of
# points; dates below the floor are dropped from the series rather than averaged in. Five
# indices: the floor is the one `01_feasibility_analysis` checks, twice the largest top-k the
# backtest sweep declares, so that a long leg and a short leg both exist to rank.
# A time-series information coefficient needs enough rows WITHIN one instrument, not enough
# instruments on a date. Two indices are not a cross-section - a rank correlation over two
# points is +1 or -1 and carries no information - so the screen in section 11 is per index and
# over time, and this is its floor. One validation fold of this bot is about 125 sessions, so
# 60 admits every fold and would refuse a fold cut in half by missing fills.
MIN_ROWS_PER_SYMBOL = 60
# Which session panel this workspace's label lives on (setup.yaml::labels.spec_of). The two
# specs decide six hours apart, so their panels are different aggregations of the same bars
# and the three models below would fit different series under the same names.
...
# The market-regime model is given the shortest close-to-close volatility window the
...
MKT_VOL_WINDOW = int(min(SETUP["features"]["windows"]["close_to_close_volatility"]))
MKT_VOL_COL = f"mkt_vol_{MKT_VOL_WINDOW}d"
...
# the 5PM rollover - and closed on the decision bar, the last bar that closes at or before the
# session's declared close (`decision.snapshot`, the 16:00-20:00 UTC bar on this server).
# `_features.session_panel` is the one function that does it, the same one `02_labels`
# sealed the labels on and `03_financial_features` built its matrix on, so the three agree on
# which session a bar belongs to and on the price a session closes at.
...
assert SETUP["decision"]["session_calendar"] == SESSION_CALENDAR

# %% [markdown] tags=[]
# ### Select the Universe
#
# The universe is the one declared in `setup.yaml`. The labels were built for that
# list, so an index present in the price file but absent from the declared universe
...
print(f"Loaded: {n_symbols} indices on the {SPEC!r} panel, {len(dates)} sessions")
print(f"Period: {dates[0]} to {dates[-1]}")

# %% [markdown] tags=[]
# ### What Is In This Universe
#
# Two indices, and the count is not enough to read the rest of the notebook because the three
# models treat them differently along lines the count hides.
#
# `US500` and `USTEC` are the same economy read at two breadths - five hundred names against a
# hundred technology-weighted ones - and they correlate about 0.9 in session returns. That
# matters twice below. In section 4 the state-space model is fitted separately for each index, so
# the correlation costs nothing: two fits, two sets of noise parameters. In section 5 the
# market-regime model is fitted on the average of the two, and the correlation is why one average
# is enough: there is no second factor here to lose by averaging, unlike the five-way dollar
# average this notebook is forked from, where the sign of each leg had to be fixed first.
#
# The table carries what the later sections depend on: how many sessions each index has, so the
# minimum training length in section 4 can be checked against it, and how far a session travels,
# which is the quantity the state-space model has to attribute between a moving level and quoting
# noise. Scale is why the models read the logarithm of the price rather than the price: `USTEC`
# quotes about four times `US500`, and one point means something different in each.
...
universe_table = (
    ...
    .agg(
        ...
        pl.col("close").median().round(0).alias("median_level"),
...
#
# A walk-forward fold is an index of date ranges: a training range the model may be fitted
...
# %% [markdown] tags=[]
# The gaps the second check measures, one row per variant label and fold. Every one has to be
# positive, and the narrowest is the one to watch: it is the label whose validation opens closest
# to the sessions its features were fitted on.
#
# **This bot declares no variant label in a workspace, and that is the design rather than an
# omission.** Its two labels sit on two different session panels (`setup.yaml::labels.spec_of`),
# so they cannot share one feature matrix and each is modelled in its own workspace with
# `variants: []`. The check therefore has nothing to measure here, and an empty result is the
# right answer rather than a missing one - which is why it is printed as such instead of being
# indexed into.

# %% tags=[]
variant_gaps = pl.DataFrame(assert_variant_folds_are_out_of_sample(CASE_STUDY_ID, PRIMARY_LABEL))
if variant_gaps.height:
    ...
    print(
        f"No variant label in this workspace: it models {PRIMARY_LABEL} alone, on the "
        f"{SPEC!r} session panel. The other spec is modelled in its own workspace on its own "
        "panel, so there is no second label whose folds could overlap these."
    )
...
# market-regime model's emissions and transitions, and the return model's coefficients
...
show_plotly_with_alt(
    ...
    "estimated on and the later stretch they are applied to out of sample. The four "
    "validation bars step up and to the right, fold zero covering the most recent window "
    "and fold three the oldest, and all four end to the left of the dashed vertical rule "
...
# ### Run It Fold by Fold
#
# For each fold and each index, the noise sizes are fitted on the training sessions and
...
print(f"\nKalman features: {len(kalman_df):,} rows, {n_symbols} indices x {len(folds)} folds")
...
# Deleting the second half of one index's history and re-running gives a direct answer.
...
# %% [markdown] tags=[]
# The model reads one series: an equal-weight average return across the declared universe,
# which here is the two US index CFDs. **No sign is fixed**, and the paragraph that stood here
# claiming otherwise - "an average dollar return across the indices that have the dollar on one
# side of the quote, which here is all five", with `USDJPY` and `EURUSD` as the worked example -
# was the FX template's and was deleted on 2026-09-08 (third pass): this universe has two
# instruments, both quoted in dollars, both rising and falling together. The legs are derived
# from the declared universe rather than listed here, so a universe change cannot silently drop
# one, and the code cell below asserts the list is non-empty and prints it.

# %% tags=[]
MARKET_LEGS = list(SYMBOLS)
assert MARKET_LEGS, "no instrument to build the market factor from"
print(f"Index-market factor legs: {MARKET_LEGS} (equal weight, no sign to fix)")

daily_rets = prices.with_columns(
    (pl.col("close") / pl.col("close").shift(1).over("symbol") - 1).alias("ret")
).drop_nulls(subset=["ret"])

# No leg is negated. The five-way dollar average this notebook is forked from had to sign each
# pair, because EURUSD rising and USDJPY rising are opposite moves in the dollar; two US equity
# indices quoted in dollars rise and fall together, so the average is a plain one. That is a fact
# about the universe rather than a simplification - the assertion above and the correlation
# reported in section 1 stand behind it - and a universe that later added a short leg would need
# this cell changed rather than being silently averaged.
mkt_daily = (
    daily_rets.group_by("timestamp").agg(pl.col("ret").mean().alias("mkt_ret")).sort("timestamp")
)

# %% [markdown] tags=[]
# The model is given two numbers per session rather than one: the average index return and a
# rolling standard deviation of it over the window bound above. The return alone would let the
# model separate the states only through how far individual sessions scatter, and the rolling
# figure states the recent scale directly, which is the quantity the two states differ in.

# %% tags=[]
mkt_daily = mkt_daily.with_columns(
    pl.col("mkt_ret").rolling_std(MKT_VOL_WINDOW).alias(MKT_VOL_COL)
)

print(f"Index-market factor series: {len(mkt_daily):,} sessions")
...
# %% tags=[]
valid_mkt = mkt_daily.drop_nulls(subset=["mkt_ret", MKT_VOL_COL]).filter(
    pl.col("timestamp") < HOLDOUT_START
)
valid_dates = valid_mkt["timestamp"].to_list()
_native = valid_mkt.select(["mkt_ret", MKT_VOL_COL]).to_numpy()
mkt_arr = _native * HMM_SCALE
full_mkt = mkt_daily.drop_nulls(subset=["mkt_ret", MKT_VOL_COL])
full_dates = full_mkt["timestamp"].to_list()
full_arr = full_mkt.select(["mkt_ret", MKT_VOL_COL]).to_numpy() * HMM_SCALE
...
scaled_var = float(mkt_arr[:, 0].var())
...
for fold in folds:
    _dates, _arr = (
        (full_dates, full_arr) if fold["fold"] == HOLDOUT_FOLD_ID else (valid_dates, mkt_arr)
    )
...
seal_model, _, _ = fit_best_hmm(mkt_arr[seal_train_idx])
seal_obs = mkt_arr[seal_train_idx]
cut = len(seal_obs) // 2
hmm_drift = float(
    np.abs(
...
# ARIMA(1,0,1) fitted to an index's daily returns says that today's return is partly
...
# windows roll forward one year at a time and overlap by two of their three years, so the
...
# across indices so one badly behaved pair does not stand for the fold. The axis is
...
# produces. Within a fold the column still ranks indices; across folds it is not one scale,
...
#
# The return model is refitted per index and its two coefficients have no market-level
...
print("Observation noise R, median across indices, by fold:")
print(
...
fig.update_layout(
    title=(
        ...
        "<br><sup>Left: median across indices of the three noise sizes, on a log axis."
...
show_plotly_with_alt(
    ...
    "market-regime state in sessions: the calm state starts far higher, falls steeply "
...
# are fitted per index, so their rows are keyed by pair, session and fold. The market-regime
...
# check.
#
# One gap is expected here and its shape is known. The market-regime model needs a
...
record = write_artifact(
    ...
    written_by="case_studies/exness_usidx_sess/04_model_based_features.py",
    inputs={
        "load_mt5_bars:4h:sessions": value_digest(prices),
...
#
# The validation windows are contiguous - fold 3's ends the session before fold 2's begins
...
if "hmm_regime_prob_high_vol" in eval_features.columns:
    hmm_validation = (
        ...
        .sort("timestamp")
        .drop_nulls()
        .join(mkt_daily.select("timestamp", MKT_VOL_COL), on="timestamp", how="left")
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=hmm_validation["timestamp"],
            y=(hmm_validation[MKT_VOL_COL] * 100),
            mode="lines",
    ...
    fig.update_yaxes(
        title_text=f"Index-market volatility, {MKT_VOL_WINDOW}d (%)",
        rangemode="tozero",
        row=2,
        col=1,
    )
...
# %% [markdown] tags=[]
# ## 11. Does Any of This Predict, Index by Index?
#
# `exness_fx_d1` asks whether each column can order five indices on a session. That question
# cannot be asked here and the reason is not statistical delicacy: a rank correlation over
# **two** points is +1 or -1 by construction, whatever the numbers are, so a cross-sectional
# information coefficient on this universe would be a coin flip reported to four decimals.
#
# The question this bot actually asks its features is the time-series one - `mapping.class` is
# `time_series_threshold`, each index is compared with its own score distribution - so the
# screen is the time-series information coefficient: within one index, over that index's own
# validation rows, does the column's rank correlate with the next session's return? It is
# reported per index, because with two of them an average would hide a disagreement rather
# than summarise one, and it is a diagnostic rather than a decision: nothing is selected or
# dropped on it.
#
...
# %% [markdown] tags=[]
# The join above indices each validation row with the return the label file records for it,
...
#
# A correlation coefficient reported on its own carries no sampling variation, and this one
# has to carry a great deal: consecutive sessions of an overlapping forward label share part
# of their outcome window, so an ordinary standard error would be too small. The statistic is
# therefore built from its own per-observation contributions - rank the feature and the label
# **within** an index, standardise both, and multiply - whose mean over that index's rows is
# exactly its Spearman coefficient and whose series a Newey-West correction can widen by the
# amount the overlap implies. Building it this way rather than calling a correlation function
# is what keeps the uncertainty in view.
#
# The pooled series that goes into the correction is the average of the two indices'
# contributions on each session. The per-index coefficients are reported beside it and are the
# thing to read first: two indices that disagree in sign are not one weak signal.
#
# A column that is constant within an index - the two market-regime columns take one value per
# session and are the same for both - has no ranking to correlate and is skipped rather than
# returned as a NaN. One NaN reaching the next cell becomes a NaN adjusted p-value for **every**
# column, because the multiple-testing procedure sorts the whole family together, and the
# screen would then report nothing found, which is what a genuine negative result also looks
# like.
...
for feature in temporal_feature_cols:
    # The per-observation contribution to a Spearman correlation, formed WITHIN each index over
    # its own validation rows: normalise the feature and the label to ranks inside that index,
    # centre and scale each to unit variance, and multiply. The mean of that product over an
    # index's rows IS that index's Spearman information coefficient, and the series of it is
    # something compute_ic_hac_stats can correct for overlap the same way it corrects a
    # cross-sectional IC series - which is the whole reason for building the statistic this way
    # rather than calling a correlation function and losing access to its sampling variation.
    parts = []
    for symbol in SYMBOLS:
        sub = eval_df.filter(pl.col("symbol") == symbol).select("timestamp", feature, label_col)
        sub = sub.drop_nulls().drop_nans()
        if sub.height < MIN_ROWS_PER_SYMBOL:
            continue
        ranks = sub.with_columns(
            pl.col(feature).rank("average").alias("_rf"),
            pl.col(label_col).rank("average").alias("_rl"),
        )
        zf = (pl.col("_rf") - pl.col("_rf").mean()) / pl.col("_rf").std(ddof=0).clip(lower_bound=1e-12)
        zl = (pl.col("_rl") - pl.col("_rl").mean()) / pl.col("_rl").std(ddof=0).clip(lower_bound=1e-12)
        product = ranks.with_columns((zf * zl).alias("ic")).select(
            "timestamp", pl.lit(symbol).alias("symbol"), "ic"
        )
        # A column that is constant inside an index has no ranking to correlate; its rank
        # standard deviation is zero and the product is meaningless rather than zero.
        if float(ranks["_rf"].std(ddof=0) or 0.0) < 1e-12:
            continue
        per_symbol_ic_rows.append(
            {
                "feature": feature,
                "symbol": symbol,
                "n": product.height,
                "ic": float(product["ic"].mean()),
            }
        )
    ...
    if len(pooled) >= 20:
        temporal_ic[feature] = compute_ic_hac_stats(
            pooled, ic_col="ic", label_horizon=LABEL_HORIZON_SESSIONS
        )
per_symbol_ic = (
    pl.DataFrame(per_symbol_ic_rows)
    .pivot(on="symbol", index="feature", values="ic")
    .sort("feature")
    if per_symbol_ic_rows
    else pl.DataFrame(schema={"feature": pl.String})
)
...
#
# A run over a reduced sample leaves every column below the row floor and produces an empty
# frame, which the branch below handles rather than failing.
...
if feature_names:
    ...
    print(
        f"Columns with a computable IC: {len(feature_names)} of {len(temporal_feature_cols)}"
    )
    ...
    n_fdr_sig = 0
    print(f"Screen omitted: no index reaches {MIN_ROWS_PER_SYMBOL} validation rows in this run.")
...
# %% [markdown] tags=[]
# And the same coefficients per index, which is the table to read first. A column whose two
# indices disagree in sign has not found something weak; it has found two different things, or
# nothing. `US500` and `USTEC` correlate about 0.9 in returns, so agreement here is close to
# the minimum that should be expected rather than confirmation.
...
#
# Two of the ten columns cannot appear in the chart at all. The market-regime columns take
...
# column whose interval spans zero has not shown it can rank indices on this label over this
...
if len(eval_summary):
    ...
    ic_title = (
        ...
        " screened per index; the two market-regime"
        "<br>columns take one value per session and are constant within an index."
...
if len(eval_summary):
    ...
    print(f"Chart omitted: no index reaches {MIN_ROWS_PER_SYMBOL} validation rows.")
...
#   estimated before it.
# - The market-regime model is fitted on one market-wide series, so its two columns take the
#   same value for both indices on a session. That is not a defect here the way it is in a
#   cross-sectional book - a time-series threshold can act on a market-wide state - but it does
#   mean the screen in section 11 cannot measure them, because a constant within an index has
#   no ranking to correlate. `05_evaluation` is where they are tested jointly with the rest.
...
#   column therefore ranks indices by the noise their own fit estimated, not by anything that
#   happened in the window being scored.
# - The return model's order is fixed at $(1,0,1)$ for every index and every fold rather than
#   chosen per index, so its error measures surprise relative to one assumed dynamic.
# - The whole artifact belongs to ONE spec. The other spec's workspace writes a different file
#   under the same path, because its panel is a different aggregation of the same bars; the
#   digest is what tells the two apart.
...
if len(eval_summary):
    ...
    print(f"Columns with a computable time-series IC: {len(feature_names)}")
    ...
    print(
        f"Screen omitted: a time-series IC needs {MIN_ROWS_PER_SYMBOL} validation rows within "
        "one index and no index in this run reaches that."
    )
