...
def fixed_threshold_signal(
    ...
    threshold_convention: Literal["probability_mirror", "signed"] = "probability_mirror",
) -> pl.DataFrame:
    ...
    For regression predictions, the score may need normalization first - or the
    caller must say so with ``threshold_convention="signed"``. See below.

    ``threshold_convention`` decides where the SHORT threshold of a ``long_short``
    book sits, and it is the difference between a book that can be flat and one
    that cannot:

    * ``"probability_mirror"`` (the default, and what this function has always
      done): the short threshold is ``1 - threshold``, the mirror of the long
      threshold about 0.5. That is correct for a probability in [0, 1] - at
      ``threshold=0.7`` the book is long above 0.7, short below 0.3 and flat in
      between - and it is WRONG for any score that does not live on [0, 1]. A
      regression score of order 1e-3 is below ``1 - 0.0008`` on every observation,
      so every non-long row becomes a SHORT and the book is invested 100% of the
      time. That is a different strategy from the one such a caller means, and it
      was found in production (``bots/exness_usidx_sess/BOT.md``, phase 5).
    * ``"signed"``: the short threshold is ``-threshold``, i.e. long above ``+t``,
      short below ``-t``, flat in the band between them. This is the convention a
      REGRESSION score in the units of the label needs, and ``threshold=0.0``
      under it is the sign of the forecast (still invested whenever the score is
      non-zero, which is the declared meaning of a zero threshold, not a defect).

    The default is unchanged so that every existing caller keeps its behaviour;
    a caller whose score is not a probability has to say so.
    ...
    Args:
        ...
        signal_type: "long_only" (signal=1 when above threshold) or
                     "long_short" (signal=1 above, signal=-1 below the short threshold)
        threshold_convention: how the short threshold of a long_short book is
                     derived from ``threshold`` - "probability_mirror" (1 - t) or
                     "signed" (-t). Ignored by "long_only", which has no short leg.

    Returns:
        DataFrame with added 'signal' column (-1, 0, or 1)
    """
    if threshold_convention not in ("probability_mirror", "signed"):
        msg = (
            f"Unknown threshold_convention: {threshold_convention!r} "
            "(expected 'probability_mirror' or 'signed')"
        )
    ...
    if signal_type == "long_only":
        ...
        lower_threshold = (
            1.0 - threshold if threshold_convention == "probability_mirror" else -threshold
        )
        return predictions.with_columns(
...
def build_target_weights_from_config(
    ...
    Config dict keys:
        ...
        threshold_convention (str): For fixed threshold with long_short=True -
            "probability_mirror" (default, short below 1-t) or "signed" (short
            below -t, flat in the band). A regression score needs "signed".
    ...
    if method in ("equal_weight_top_k", "score_weighted_top_k", "inverse_vol"):
        ...
        df_with_signal = fixed_threshold_signal(
            ...
            score_col=score_col,
            signal_type=signal_type,
            # Part of the identity: a config that does not name it gets the historical
            # probability-mirror short threshold, so no existing spec changes hash or meaning.
            threshold_convention=config.get("threshold_convention", "probability_mirror"),
        )
        return _apply_direction(_signals_to_equal_weights(df_with_signal, time_col, asset_col))
