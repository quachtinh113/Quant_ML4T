...
def build_resolved_backtest_config(
    ...
    *,
    ...
    if share_type:
        # mt5_lot (case_studies/utils/backtest_runner.py::_apply_lot_floor_if_declared, guard d)
        # is a WEIGHT-level unit convention: the lot floor is rounded down / dropped on the
        # weights frame BEFORE it reaches the engine ("weights =
        # _apply_lot_floor_if_declared(...)" in run_backtest). ml4t.backtest.ShareType has only
        # FRACTIONAL and INTEGER members (ml4t/backtest/config.py); passing "mt5_lot" through
        # unchanged raises ValueError("'mt5_lot' is not a valid ShareType") at engine-config
        # build time (found while wiring exness_fx_d1's generation 3, 2026-09-10,
        # bots/exness_fx_d1/GEN3_DECLARATION_2026-09-10.md).
        #
        # Mentor fleet gate, 2026-09-10, item B: maps to "fractional", not "integer". The lot
        # grid (volume_step) is ALREADY the discretisation enforced upstream by
        # _apply_lot_floor_if_declared (it returns a weight sized to an exact multiple of one
        # MT5 lot); a SECOND, INCOMPATIBLE discretisation at the engine's own share level
        # (shares = notional / price, rounded to a whole unit under ShareType.INTEGER) is not a
        # broker constraint the lot grid didn't already enforce -- it is a residual rounding
        # error the fleet gate measured and declared as a known, conservative discretisation of
        # generation 3 (GEN3_DECLARATION_2026-09-10.md, "Mentor fleet-gate addendum", item 1):
        # for a base-currency-is-account-currency symbol (USDJPY/USDCAD) the engine's own
        # ShareType.INTEGER truncation toward zero (ml4t/backtest/preopen.py, measured read-only,
        # 2026-09-10) shrinks a lot-floor-correct weight by up to ~10% at the 0.01-lot floor and
        # <=0.3% at real registered size; "fractional" removes that second truncation entirely
        # for the NEXT registered run (this generation, already running, keeps "integer").
        # Every other declared share_type ("integer", "fractional") passes through unchanged.
        preset["position_sizing"]["share_type"] = "fractional" if share_type == "mt5_lot" else share_type
    ...
    if share_type == "mt5_lot":
        # Mentor fleet gate, 2026-09-10, item B. Two things this block does, both required by
        # the gate: (1) records the DECLARED share_type ("mt5_lot"), not the translated engine
        # value ("fractional") that `position_sizing.share_type` above now carries -- a reader of
        # the resolved config must be able to tell the two apart. (2) folds the lot-sizing
        # declaration itself (sizing.min_lot_policy, costs.contract.*) into `metadata`, which
        # `EngineBacktestConfig.to_dict()["metadata"]` carries into `backtest_config` and so INTO
        # THE HASHED strategy_spec (case_studies/utils/backtest_presets.py::
        # serializable_backtest_spec hashes the whole spec, this function's caller
        # `ensure_backtest_spec` is what assembles `backtest_config` from this dict) -- before
        # this, toggling `sizing.min_lot_policy` alone did not move `backtest_hash` (declared and
        # closed in GEN3_DECLARATION_2026-09-10.md, "Mentor fleet-gate addendum", item 2). A case
        # study that has not opted into mt5_lot never enters this branch, so its resolved spec
        # (and hash) is byte-identical to before this change.
        from case_studies.utils.backtest_loaders import read_mt5_lot_sizing_declaration

        metadata["execution_share_type_declared"] = "mt5_lot"
        metadata["mt5_lot_sizing"] = read_mt5_lot_sizing_declaration(case_study)
