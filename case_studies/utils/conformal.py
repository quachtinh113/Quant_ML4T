...
HOLDOUT_CONFORMAL_EMBARGO_STEPS: dict[str, int] = {
    ...
    # exness_fx_d1 (bots/exness_fx_d1/BOT.md) keeps the fx_pairs labels block: a session panel
    # with one row per CME_FX session, so each label's horizon in data steps is its own
    # session count. Declares the same conformal_weighted allocator as fx_pairs.
    "exness_fx_d1/fwd_ret_1d": 1,
    "exness_fx_d1/fwd_ret_5d": 5,
    "exness_fx_d1/fwd_ret_21d": 21,
    # exness_gold_sess (bots/exness_gold_sess/BOT.md) runs a SESSION panel: two decision slots
    # a weekday, so a step of this panel is half a day and the embargo is counted in slots, not
    # in days. fwd_ret_8h resolves at the session close, one slot; fwd_ret_24h two slots later;
    # dir_tb_8h reaches exactly as far as the return whose barriers it is the sign of.
    "exness_gold_sess/fwd_ret_8h": 1,
    "exness_gold_sess/fwd_ret_24h": 2,
    "exness_gold_sess/dir_tb_8h": 1,
    # exness_usidx_sess (bots/exness_usidx_sess/BOT.md) runs TWO session panels, one per label,
    # and each panel has one row per index per NYSE cash session. Both labels resolve at the
    # next decision instant on their own panel - the cash close for the intraday label, the
    # next cash open for the overnight one - so on their own grid each is exactly one step.
    "exness_usidx_sess/fwd_ret_intraday": 1,
    "exness_usidx_sess/fwd_ret_overnight": 1,
