...
avoid_windows:
  ...
  - name: "Sự kiện chính trị Anh"
    when: "Ngày bầu cử, ngân sách"
news:
  - name: "BoE"
    when: "8 lần/năm, 12:00 UTC (mùa hè 11:00)"
  - name: "CPI, GDP, việc làm Anh"
    when: "07:00 UTC (mùa hè 06:00)"
  - name: "PMI Anh"
    when: "09:30 UTC (mùa hè 08:30)"
  - name: "NFP"
...
# GBPUSD — hồ sơ phiên và giờ giao dịch

**Lớp tài sản**: forex · **Bot**: `exness_fx_d1` · **Khuôn case study**: fx_pairs

**Vai trò trong danh mục**: Biến động cao hơn EURUSD cùng phiên London; nhạy với tin Anh
...
- Momentum và mean reversion nến D1
- Carry BoE và Fed
- Biến động thực hiện cao hơn nhóm, cần chuẩn hóa theo vol (Ch08 01)

## 8. Bằng chứng và mẫu trong repo

- `case_studies/fx_pairs/config/setup.yaml` có GBP_USD trong universe 20 cặp G10
...
mt5.initialize()
s = "GBPUSD"  # đổi sang tên đúng trong Market Watch
...
|---|---|
| Tên trong Market Watch | `GBPUSDm` (suffix `m`; `path` = `Standard\Forex\GBPUSDm`) |
...
| Tiền tệ base / profit / margin | GBP / USD / GBP |
...
**Spread theo phiên (bps của mid, 30 ngày tick `COPY_TICKS_INFO`, 2026-08-06 → 2026-09-05, 1,108,305 tick):**

| Phiên | Số tick | p50 (bps) | p90 (bps) | p50 (points) | p90 (points) |
|---|---|---|---|---|---|
| all | 1,108,305 | 0.74 | 0.74 | 10 | 10 |
| asia | 280,554 | 0.74 | 0.74 | 10 | 10 |
| london | 246,541 | 0.74 | 0.74 | 10 | 10 |
| overlap | 331,752 | 0.74 | 0.74 | 10 | 10 |
| new_york | 197,738 | 0.74 | 0.74 | 10 | 10 |
| rollover | 18,698 | 0.74 | 0.74 | 10 | 10 |
| other | 33,022 | 2.44 | 7.6 | 33 | 103 |
