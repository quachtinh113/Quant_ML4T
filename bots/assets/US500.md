...
# US500 — hồ sơ phiên và giờ giao dịch
...
**Vai trò trong danh mục**: Beta cổ phiếu Mỹ; rộng nhất, rẻ nhất trong nhóm chỉ số

**Hợp đồng**: Thường 1 đơn vị chỉ số mỗi lot trên Exness; tên mã có thể là US500 / SPX500 / SP500; xác minh `trade_contract_size` và tên trong Market Watch
...
- Lợi nhuận qua đêm và trong phiên tách riêng (Ch08 01)
- Biến động thực hiện và regime (Ch09)
- Cờ phiên, cờ mép phiên, cờ FOMC và opex
- Feature từ VIX nếu tài khoản có (VIX là CFD riêng), cấu trúc kỳ hạn ES nếu lấy được (Ch02 06)

## 8. Bằng chứng và mẫu trong repo

- ES trong `case_studies/cme_futures`
- SPY trong `case_studies/etfs`
- `26_mlops_governance/04_circuit_breakers.py` chạy trên SPY 2020 H1 — mẫu breaker
...
mt5.initialize()
s = "US500"  # đổi sang tên đúng trong Market Watch
...
|---|---|
| Tên trong Market Watch | `US500m` (suffix `m`; `path` = `Standard\Indices\US500m`) |
| `trade_contract_size` | 1 |
| `volume_min` / `volume_step` / `volume_max` | 0.14 / 0.01 / 1000.0 |
| `digits` / `point` | 2 / 0.01 |
| `swap_long` / `swap_short` (`swap_mode` 1 = points/lot/đêm) | -147.4 / 0.0 |
...
| Lịch sử H4 | 11,218 nến, 2019-07-16 → 2026-09-04 |
| Lịch sử H1 | 22,554 nến, 2022-10-25 → 2026-09-04 |
