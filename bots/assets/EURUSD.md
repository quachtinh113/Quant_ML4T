...
news:
  - name: "ECB"
    when: "8 lần/năm, quyết định 13:15 UTC, họp báo 13:45 (mùa hè 12:15/12:45)"
  - name: "PMI flash Eurozone"
    when: "~09:00 UTC (mùa hè 08:00)"
  - name: "Số liệu Đức (ZEW, IFO, CPI)"
    when: "07:00-10:00 UTC"
  - name: "NFP"
...
# EURUSD — hồ sơ phiên và giờ giao dịch

**Lớp tài sản**: forex · **Bot**: `exness_fx_d1` · **Khuôn case study**: fx_pairs

**Vai trò trong danh mục**: Cặp thanh khoản cao nhất thế giới, spread hẹp nhất; neo của cross-section FX
...
- Momentum 1/3/6 tháng trên nến D1 (Ch08 01)
- Carry từ chênh lệch lãi suất ECB và Fed (Ch08 04, FRED)
- Chỉ số USD làm feature cấu trúc (Ch08 03)
- Biến động thực hiện và z-score cross-section (`fx_pairs` 03)

## 8. Bằng chứng và mẫu trong repo

- `case_studies/fx_pairs/config/setup.yaml` có EUR_USD trong universe 20 cặp G10
...
mt5.initialize()
s = "EURUSD"  # đổi sang tên đúng trong Market Watch
...
|---|---|
| Tên trong Market Watch | `EURUSDm` (suffix `m`; `path` = `Standard\Forex\EURUSDm`) |
...
| Tiền tệ base / profit / margin | EUR / USD / EUR |
...
**Spread theo phiên (bps của mid, 30 ngày tick `COPY_TICKS_INFO`, 2026-08-06 → 2026-09-05, 753,280 tick):**

| Phiên | Số tick | p50 (bps) | p90 (bps) | p50 (points) | p90 (points) |
|---|---|---|---|---|---|
| all | 753,280 | 0.69 | 0.69 | 8 | 8 |
| asia | 170,663 | 0.69 | 0.69 | 8 | 8 |
| london | 173,085 | 0.69 | 0.69 | 8 | 8 |
| overlap | 252,751 | 0.69 | 0.69 | 8 | 8 |
| new_york | 127,350 | 0.69 | 0.69 | 8 | 8 |
| rollover | 11,033 | 0.69 | 0.69 | 8 | 8 |
| other | 18,398 | 0.69 | 4.34 | 8 | 50 |
