"""Measure spread by session and re-read symbol_info for XAUUSD and XAGUSD (Exness MT5).

Preparation task for phase 0-1 of ``exness_gold_sess`` (2026-09-07). Windows only: the
...
env does not build on Windows, ``docs/installation.md:115-118``)::
    ...
    PYTHONPATH=<repo root> uv run --no-project --python 3.12 \
        ...
        python bots/exness_gold_sess/tools/measure_metals_costs.py
...
3. runs :func:`bots._shared.costs_mt5.measure_spreads` over ``DAYS`` days of
   ``COPY_TICKS_INFO`` ticks for the two metals, the same call the five FX pairs of
   ``exness_fx_d1`` were measured with on 2026-09-05;
...
5. backs up ``spreads_by_session.{json,parquet}`` as ``*.bak_<STAMP>`` and rewrites them
   with the existing rows **unchanged** plus the two metal symbols appended. Every row of
   the parquet carries its own measurement window, so the FX rows keep their 2026-09-05
   window and the metals rows carry today's. The JSON keeps its top-level scalars (they
...
def main() -> int:
    ...
    try:
        ...
        out = DATA_DIR / f"symbol_info_metals_{STAMP}.json"
        out.write_text(
            json.dumps(
                {
